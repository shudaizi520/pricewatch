...
3. 确认后检查首次飞书消息只收到一次。再点击“立即检查”，价格及状态未变化时应保持安静。当前开发机无法代你发送真实飞书消息或访问你的 TrueNAS 网络。
...
- 用 `docker stats` 记录真实空闲内存、浏览器检查峰值；用 `docker image ls` 记录镜像大小。此开发环境没有 Docker，故不提供未经测量的数字。
